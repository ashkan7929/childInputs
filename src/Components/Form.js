...
const fieldMeetsCondition = (values) => (field) => {
    ...
    if (field.conditional && field.conditional.length > 0) {
        for (let index = 0; index < field.conditional.length; index++) {
            const segments = field.conditional[index].field.split("_");
            const fieldId = segments[segments.length - 1];
            segmentsShows.push(values[fieldId] === field.conditional[index].value)
            console.log(segmentsShows);
            // return values[fieldId] === field.conditional[index].value;
        }
        return segmentsShows.every(segment => segment === true) && true
    }
    return true;
};
                ...
                .map((field) => {
                    switch (field.component) {
                        ...
                        case "drop":
                            return (
                                <div>
                                    <label for="cars">{field.label}</label>
                                    <select name="cars" id="cars" onChange={(e) => fieldChanged(field._uid, e.target.value)}
                                        value={values[field._uid]}>
